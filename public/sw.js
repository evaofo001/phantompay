...
function doBackgroundSync() {
  ...
  return fetch('/api/sync-offline-transactions', {
    method: 'POST',
    body: JSON.stringify(getOfflineTransactions()),
    headers: {
...
self.addEventListener('push', event => {
  const options = {
    body: event.data ? event.data.text() : 'New notification from PhantomPay',
  ...
  event.waitUntil(
    self.registration.showNotification('PhantomPay', options)
  );
});
